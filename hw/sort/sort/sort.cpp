...
//  sort
//
//  Created by Weijia Shi on 8/9/17.
...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    for(int unsort = 1; unsort < s.size(); unsort++)
    {
        Sensor temp = s[unsort];
        int loc = unsort;
        while (loc > 0 && !comp(s[loc - 1], temp))
        {
            s[loc] = s[loc-1];
            loc--;
        }
...
int main()
{
    ...
    // destruction time.
    {
        ...
        for (size_t k = 0; k < auxPointers.size(); k++)
            sensors[k] = *auxPointers[k];
